...
<body>
                    ...
                    <form method="POST" id="settleForm">
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <label class="form-label fw-bold small text-secondary text-uppercase mb-0" style="letter-spacing: 0.05em;">Settlement Cash Tendered Today</label>
                                <span class="badge bg-light text-secondary border small">Partial / Zero allowed with Pay Later</span>
                            </div>
                            <div class="input-group input-group-lg">
                                <span class="input-group-text bg-transparent border-secondary-subtle fw-bold" style="color: var(--gold-primary);">Rs.</span>
                                <input type="number" name="cash_received" id="cashReceived" class="form-control fw-bold border-secondary-subtle fs-4" min="0" placeholder="0.00" value="0" oninput="calculateSettle()">
                            </div>
    ...
    <script>
        ...
        function submitReturnCheckout(isPayLater) {
            ...
            document.getElementById('actionTypeInput').value = isPayLater ? 'pay_later' : 'settle';
        ...
        function calculateSettle() {
            ...
            if (balance >= 0) {
                ...
                btnSubmitSettle.style.display = 'block';
                btnSubmitSettle.disabled = false;
                btnSubmitSettle.className = 'btn btn-luxury-gold btn-lg w-100 fw-bold shadow rounded-pill py-3';
                btnSubmitSettle.innerHTML = '<i class="fa-solid fa-check-double me-2"></i> Finalize Return & Print Settlement';
                
                btnPayLater.style.display = 'none';
                btnPayLater.disabled = true;
            } else {
                const deficit = Math.abs(balance);
                balBox.style.backgroundColor = 'rgba(245, 158, 11, 0.12)';
                balBox.style.borderColor = 'rgba(245, 158, 11, 0.4)';
                balLabel.innerText = 'Unpaid Deficit (Due from Client):';
                balLabel.className = 'fw-bold small text-warning';
                dispBal.innerText = 'Rs. ' + deficit.toFixed(2);
                dispBal.className = 'fw-bold fs-4 text-warning';

                btnSubmitSettle.style.display = 'none';
                btnSubmitSettle.disabled = true;

                btnPayLater.style.display = 'block';
                btnPayLater.disabled = false;
                btnPayLater.className = 'btn btn-warning btn-lg fw-bold shadow rounded-pill py-3 px-4 text-dark w-100';
                btnPayLater.innerHTML = '<i class="fa-solid fa-clock-rotate-left me-2"></i> Pay Later (Rs. ' + deficit.toFixed(2) + ' Due)';
            }
        }
